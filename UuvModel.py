...
class UuvParas(object):
    ...
    def __init__(self, g=9.8, tim_sample=0.01,
                 uuv_b=14671, uuv_m=1840, uuv_dis_c=np.array([0.02, -0.005, 0.008]),
                 uuv_j=np.array([289.3, 6771.8, 6771.8]), uuv_lu=1019.2, uuv_s=0.224, uuv_l=7.738,
                 uuv_cx=0.141, uuv_mx_beta=0.00152, uuv_mx_dr=-0.000319, uuv_mx_dd=-0.0812, uuv_mx_wx=-0.0044,
                 uuv_mx_wy=0.0008, uuv_mx_xp=0,
                 uuv_cy_alfa=2.32, uuv_cy_de=0.51, uuv_cy_wz=1.17, uuv_my_beta=0.69, uuv_my_dr=-0.11, uuv_my_wx=0,
                 uuv_my_wy=-0.61,
                 uuv_cz_beta=-2.32, uuv_cz_dr=-0.2, uuv_cz_wy=-1.17, uuv_mz_alpha=0.69, uuv_mz_de=-0.28,
                 uuv_mz_wz=-0.61,
                 uuv_add_m_k11=0.0222, uuv_add_m_k22=1.1096, uuv_add_m_K44=0.1406, uuv_add_m_k55=3.8129,
                 uuv_add_m_k26=-0.363):
...
class UuvModel(object):
    ...
    def dynamic_basic(self, state, action, debug=False):
        ...
        ms = np.array([
            ...
            p.MyBeta * 0.5 * p.Lu * p.S * p.L * v2 * beta + p.MyWy * 0.5 * p.Lu * p.S * (
                np.square(p.L)) * wy * v + p.MyDr * 0.5 * p.Lu * p.S * p.L * v2 * dr,
            p.MzAlpha * 0.5 * p.Lu * p.S * p.L * v2 * alfa + p.MzWz * 0.5 * p.Lu * p.S * (
                np.square(p.L)) * wz * v + p.MzDe * 0.5 * p.Lu * p.S * p.L * v2 * de
        ])
        # floating force
        ...
        ft = np.array([
            [-p.M * (vz * wy - vy * wz + p.Dis_c[1] * wx * wy + p.Dis_c[2] * wx * wz - p.Dis_c[0] * (
                        np.square(wy) + np.square(wz)))],
            [-p.M * (vx * wz - vz * wx + p.Dis_c[2] * wy * wz + p.Dis_c[0] * wy * wx - p.Dis_c[1] * (
                        np.square(wz) + np.square(wx)))],
            [-p.M * (vy * wx - vx * wy + p.Dis_c[0] * wz * wx + p.Dis_c[1] * wz * wy - p.Dis_c[2] * (
                        np.square(wx) + np.square(wy)))]
        ])
        mt = np.array([
            [-p.M * (p.Dis_c[1] * (vy * wx - vx * wy) + p.Dis_c[2] * (vz * wx - vx * wz)) - (
                        p.J[2] - p.J[1]) * wy * wz],
            [-p.M * (p.Dis_c[2] * (vz * wy - vy * wz) + p.Dis_c[0] * (vx * wy - vy * wx)) - (
                        p.J[0] - p.J[2]) * wz * wx],
        ...
        ftl = np.array([tl, 0, 0])
        f = np.hstack([fs.reshape([3]) + fg.reshape([3]) + ft.reshape([3]) + ftl.reshape([3]),
                       ms.reshape([3]) + mg.reshape([3]) + mt.reshape([3])])
